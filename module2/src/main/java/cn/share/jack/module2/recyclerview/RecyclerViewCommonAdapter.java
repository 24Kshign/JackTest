...
import static cn.share.jack.module2.recyclerview.wrap.WrapRecyclerAdapter.BASE_ITEM_TYPE_HEADER;

/**
 * Created by jack on 2018/5/16
 */

public abstract class RecyclerViewCommonAdapter<DATA> extends RecyclerView.Adapter<RecyclerViewCommonViewHolder> {
    ...
    public RecyclerViewCommonAdapter(Context context) {
        mContext = context;
    }

    public RecyclerViewCommonAdapter(Context context, MultiTypeAdapter<DATA> multiTypeAdapter) {
        this(context);
    ...
    @Override
    public void onBindViewHolder(RecyclerViewCommonViewHolder holder, int position, List<Object> payloads) {
        convert(holder, mDataList.get(position), position, payloads);
        setOnClickListener(holder);
    }

    private void setOnClickListener(final RecyclerViewCommonViewHolder holder) {
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (null != onItemClickListener) {
                    if (getItemViewType(holder.getLayoutPosition()) >= BASE_ITEM_TYPE_HEADER - 1) {
                        return;
                    }
     ...
     * 让子类去实现数据绑定
     */
    protected abstract void convert(RecyclerViewCommonViewHolder holder, DATA data, int position, List<Object> payloads);
