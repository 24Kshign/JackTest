...
public class ScrollChangedScrollView extends ScrollView {
    ...
    private Handler handler = new MyScrollViewHandler(this);
    ...
    private static class MyScrollViewHandler extends Handler {
        WeakReference<ScrollChangedScrollView> mWeakReference;

        MyScrollViewHandler(ScrollChangedScrollView scrollChangedScrollView) {
            this.mWeakReference = new WeakReference<>(scrollChangedScrollView);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            ScrollChangedScrollView scrollChangedScrollView = mWeakReference.get();
            if (msg.what == scrollChangedScrollView.handlerWhatId) {
                if (scrollChangedScrollView.lastY == scrollChangedScrollView.getScrollY()) {
                    ...
                    scrollChangedScrollView.handler.sendMessageDelayed(scrollChangedScrollView.handler.obtainMessage(scrollChangedScrollView.handlerWhatId
                            , this), scrollChangedScrollView.timeInterval);
